  ...
  /*  var admin = require("firebase-admin");
    ...
    var sendFCMMessage= (req, res)=>{
       ...
       /*var ref = firebase.database().ref("estate_notifications");
        ref.set({topic:input.topic , subject:input.subject, message:input.message})
        .then(()=>{
                return res.json({ success: true, message:'Notification db was set successfully',status:"OK" });
            })
        .catch((error)=>{
                console.log(error);
                res.json({error:true, message:"there was an error in setting firebase  Notifications", status:"OK"});
            });*/
        ...
        admin.messaging().sendToTopic(topic, payload)
            .then(function(response) {
                ...
                res.json({ success: true, message:'Message was sent successful', status:"OK" });
